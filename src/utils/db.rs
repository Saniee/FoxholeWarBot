...
impl Database {
    ...
    pub async fn update_guild(&self, id: i32, shard: &str, show: i32) {
        sqlx::query("UPDATE guilds SET shard = ?1, shard_name = ?2, show_command_output = ?3 WHERE id == ?4").bind(Shard::from_str(shard).api_url()).bind(Shard::from_str(shard).as_str()).bind(show).bind(id).execute(&self.conn).await.unwrap();
    }
}
