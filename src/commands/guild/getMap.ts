// prettier-ignore
import {
    SlashCommandBuilder,CommandInteraction,EmbedBuilder,Colors,AttachmentBuilder,AutocompleteInteraction,
} from 'discord.js';
...
module.exports = {
    ...
    async execute(interaction: CommandInteraction, pb: PocketBase) {
        ...
        var renderLabels =
            interaction.options.get('render-labels')?.value ?? false;
        ...
        if (statusCodeStatic == 503 || statusCodeDynamic == 503) {
            ...
            if (renderLabels == true) {
                for (var i = 0; i < dataStatic.mapTextItems.length; i++) {
                    ...
                    if (
                        dataStatic.mapTextItems[i].mapMarkerType ===
                            showLabels &&
                        showLabels != 'All'
                    ) {
                        ctx.fillText(
