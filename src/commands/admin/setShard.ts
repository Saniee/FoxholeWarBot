...
async function checkStatusCode(shardName: any) {
    const servers = [
        ...
        {
            name: 'Baker',
            url: 'https://war-service-live-2.foxholeservices.com',
        },
        {
            name: 'Charlie',
            url: 'https://war-service-live-3.foxholeservices.com',
        },
    ];
    ...
    async execute(interaction: CommandInteraction, pb: PocketBase) {
        ...
        if (record) {
            ...
            await pb
                ...
                .then(() =>
                    console.log(
                        `Updated Record for ${interaction.guild?.name}!`,
                    ),
                )
            ...
            await pb
                ...
                .then(() =>
                    console.log(
                        `Created New Record for ${interaction.guild?.name}!`,
                    ),
                )
                .catch((err) => console.log(err));
            await interaction.editReply('Saved!');
        ...
        console.log(
            `Saved option ${shardName} for ${interaction.guild?.name}!`,
        );
    },
};
